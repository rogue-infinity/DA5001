...
mdlm_shared.py — no importlib hacks needed.
...
log = build_logger("mdlm_extractor")
    ...
    if needs_hook_hidden or needs_hook_attn:
            ...
            def _make_attn_hook(layer_idx: int):
                def _attn_hook(module, inp, output):
                    ...
                    if not (
                        ...
                        log.debug(
                            "    layer %d self_attn: output[1] shape %s does not match "
                            "[%d, *, %d, %d]", layer_idx,
        ...
        if needs_hook_attn:
            if layer_attn_map:
                ...
                attentions = []
                log.warning(
                    "  attention hooks fired but captured nothing — "
...
def extract_metrics(
    ...
    with torch.no_grad():
        _, h0_states, _ = _forward_with_hooks(model, token_ids, capture_attentions=False)
    ...
    for t_idx, t in enumerate(timestep_grid.tolist()):
        ...
        losses_k:      list[float]         = []
        entropies_k:   list[torch.Tensor]  = []
        predictions_k: list[torch.Tensor]  = []
        attn_k:        list[Optional[torch.Tensor]] = []
        hidden_norm_k: list[torch.Tensor]  = []
        hidden_dir_k:  list[list[torch.Tensor]] = []
        ...
        for k in range(n_mask_configs):
            ...
            ce_full = F.nll_loss(log_p, target, reduction="none")
            losses_k.append(ce_full[masked_pos].mean().item())

            entropies_k.append(_token_entropy(logits, masked_pos))
            predictions_k.append(logits[0].argmax(dim=-1))
            attn_k.append(_attention_mean(attentions))
            if hidden:
                hidden_norm_k.append(_layer_norms(hidden))
                hidden_dir_k.append([h[0].mean(dim=0) for h in hidden])
    ...
    entropy_full_mat   = torch.stack(entropy_full_per_t)
    consistency_tensor = torch.tensor(consistency_per_t, dtype=torch.float32)
    hidden_norm_mat    = torch.stack(hidden_norm_per_t)
    cosine_sim_mat     = torch.stack(cosine_sim_per_t)
    ...
    # ── 5. Finite-difference derivatives of L(t) ─────────────────────────────
    ...
    for t_idx in grad_t_indices:
        ...
        z_t        = _apply_masking(token_ids, mask_id, t, rng=rng)
        model.zero_grad()
        for p in model.parameters():
            p.requires_grad_(True)

        with torch.enable_grad():
            logits_g, _, _ = _forward_with_hooks(model, z_t, capture_attentions=False)
...
def main() -> None:
    ...
    if not args.no_attentions:
        log.info(
            "Reloading with attn_implementation='eager' so attention weights "
            "are accessible via forward hooks (output[1] of each self_attn) …"
        )
        model, tokenizer = load_model_and_tokenizer(
            args.model, device, attn_implementation="eager"
        )
        log.debug("Model class after reload: %s", type(model).__name__)
        attn_types = {
            type(m).__name__
            for n, m in model.named_modules()
            if "self_attn" in n and "." not in n.split("self_attn")[-1].lstrip(".")
        }
    ...
    # ── Persist scalar run summary (always) ───────────────────────────────────
